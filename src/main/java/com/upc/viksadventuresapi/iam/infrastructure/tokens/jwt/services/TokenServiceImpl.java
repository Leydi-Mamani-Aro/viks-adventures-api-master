...
public class TokenServiceImpl implements BearerTokenService {
    ...
    public Jwt parseToken(String token) {
        return jwtDecoder.decode(token); // ✅ Esto devuelve un objeto Jwt con todos los claims
    }
    ...
    public String getUsernameFromToken(String token) {
        try {
            LOGGER.info("📥 Extrayendo username desde el token...");
            if (token == null || token.trim().isEmpty()) {
                LOGGER.warn("⚠️ Token nulo o vacío recibido.");
                return null;
            }
...
public String getBearerTokenFrom(HttpServletRequest request) {
    ...
    LOGGER.info("📥 Header Authorization: {}", parameter);

    if (isTokenPresentIn(parameter) && isBearerTokenIn(parameter)) {
        String token = extractTokenFrom(parameter);
        LOGGER.info("✅ Token extraído: {}", token);
        return token;
    }

    LOGGER.warn("⚠️ Token no presente o no válido (no comienza con 'Bearer ').");
    return null;
}
}
